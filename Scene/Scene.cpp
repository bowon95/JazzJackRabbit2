...
{
	// Obj 리스트에 있는 오브젝트들 업데이트 해야하기 때문에 iterator 돌려준다.
	// auto : 선언과 동시에 무조건 대입을 해주어야 한다. 
	// 대입된 타입으로 타입이 선언된다.
	// begin 시 std::list<CSharedPtr<class CGameObject>> 타입으로 선언됨.
	// auto 안쓰면 begin 시 std::list<CSharedPtr<class CGameObject>>::iterator 로 해야함.
	for (int i = 0; i < (int)ERender_Layer::Max; ++i)
	{
